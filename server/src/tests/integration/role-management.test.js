...
import { apiDelete, apiGet, apiPost, apiPut, buildApp, close, listen } from '../helpers/app.js';
...
describe('Role management and effective permissions', () => {
  ...
  it('returns the full permissions catalog with id/resource/action shape', async () => {
    ...
    const res = await apiGet(baseUrl, '/api/permissions', adminToken);
  ...
  it('lists created roles via GET /api/roles', async () => {
    ...
    const { data: created } = await (await apiPost(baseUrl, '/api/roles', adminToken, {
      name,
  ...
  it('reflects a permission grant then revoke in effective permissions', async () => {
    ...
    // WHEN - users:read is granted
    const add = await apiPut(baseUrl, `/api/roles/${created.id}/permissions/users:read`, adminToken);
    ...
    const { data: afterAdd } = await (await apiGet(baseUrl, '/api/roles', adminToken)).json();
    expect(afterAdd.find((r) => r.id === created.id).permissionsDump).toEqual(['users:read']);

    // WHEN - users:read is revoked again
    const remove = await apiDelete(baseUrl, `/api/roles/${created.id}/permissions/users:read`, adminToken);
    expect(remove.status).toBe(204);

    // THEN - effective access is revoked and the role is empty again
    expect((await apiGet(baseUrl, '/api/users', token)).status).toBe(403);
    const { data: afterRemove } = await (await apiGet(baseUrl, '/api/roles', adminToken)).json();
    expect(afterRemove.find((r) => r.id === created.id).permissionsDump).toEqual([]);
  });

  it('rejects granting a permission on a non-existent role (404)', async () => {
    // GIVEN - a role id that does not exist
    const missingId = '00000000-0000-0000-0000-000000000000';

    // WHEN - an admin grants a permission on it
    const res = await apiPut(baseUrl, `/api/roles/${missingId}/permissions/users:read`, adminToken);
  ...
  it('keeps roles isolated: managing one role does not change another role effective permissions', async () => {
    ...
    const { data: roleX } = await (await apiPost(baseUrl, '/api/roles', adminToken, {
      name: `X-${Date.now()}`,
      description: 'Concede users:read',
      permissions: ['users:read'],
    })).json();
    const { data: roleY } = await (await apiPost(baseUrl, '/api/roles', adminToken, {
      name: `Y-${Date.now()}`,
    ...
    // WHEN - role X has its users:read permission revoked
    const revoke = await apiDelete(baseUrl, `/api/roles/${roleX.id}/permissions/users:read`, adminToken);
